...
from ..models import User, Meal, WorkoutLog
...
def _day_status(cal_pct: float, protein_pct: float, workout_pct: float, is_today: bool) -> str:
    overall = (cal_pct + protein_pct + workout_pct) / 3
...
def _build_day_tracker(
    ...
    overall = round((cal_pct + protein_pct + workout_pct) / 3, 1)
    ...
    return DayTracker(
        ...
        status=_day_status(cal_pct, protein_pct, workout_pct, is_today),
    )
